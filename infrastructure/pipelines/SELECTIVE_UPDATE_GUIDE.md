...
| **UPDATE_APP** | Boolean | true/false | false | Check to update all App stacks (app_policies plus Cumulus, DataAPI, LeanRetina, Retina) |
...
### Example 1: Dry Run - Preview App Policy Changes (All App Stacks)
```
ENVIRONMENT: dev
UPDATE_APP: ✓ (checked - updates app_policies + Cumulus, DataAPI, LeanRetina, Retina)
UPDATE_CORE: (unchecked)
UPDATE_BASE: (unchecked)
DRY_RUN: ✓ (checked)
CHANGE_DESCRIPTION: "Add S3 logging permissions"
```
**Result**: app_policies and all four app stack ChangeSets are created and displayed. No changes applied.
...
### Example 3: Update Multiple Independent Stacks (with All App Types)
```
ENVIRONMENT: dev
UPDATE_CORE: ✓ (checked)
UPDATE_APP: ✓ (checked - updates Cumulus, DataAPI, LeanRetina, Retina)
UPDATE_BASE: (unchecked)
DRY_RUN: ✓ (checked)
CHANGE_DESCRIPTION: "Q1 security updates"
```
**Result**: Shows ChangeSets for Core stack and all four App stacks:
- Cumulus App stack
- DataAPI App stack
- LeanRetina App stack
- Retina App stack

(executed in order, changes not applied due to DRY_RUN=true)

### Example 4: Production Update with Safety Gates (All App Stacks)
```
ENVIRONMENT: prod
UPDATE_APP: ✓ (checked - updates all app types: Cumulus, DataAPI, LeanRetina, Retina)
UPDATE_BASE: (unchecked)
UPDATE_CORE: (unchecked)
DRY_RUN: (unchecked)
...
```
**Result**: 
1. Pre-flight check requires approval for production changes
2. ChangeSet review (shown for each app stack) requires approval
3. All four App stacks are updated sequentially
...
⚠️ **Order of updates**: If multiple stacks are selected, updates execute in this order:
   1. Core stack
   2. Base stack
   3. App stacks (if UPDATE_APP=true, updates in sequence):
      - App Policies stack (app_policies.yaml)
      - Cumulus App stack (cumulus/roles.yaml)
      - DataAPI App stack (dataapi/roles.yaml)
      - LeanRetina App stack (leanretina/roles.yaml)
      - Retina App stack (retina/roles.yaml)
